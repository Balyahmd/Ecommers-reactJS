...
function UpdateForm() {
  ...
  const handleUpdate = async () => {
    try {
      ...
      Swal.fire({
        title: 'Product gagal diperbarui',
        icon: 'error',
      })
      console.log(error);
    }
       ...
       <h1 className='text-3xl font-bold text-cyan-500'>Create Product</h1>
        <div className='grid grid-cols-5 gap-x-6 gap-y-4 my-2'>
          ...
          <div className='flex justify-end gap-5'>
            ...
            <button onClick={handleUpdate} className='bg-cyan-500 hover:bg-cyan-700 text-white font-bold py-2 px-6 rounded-lg'>Update</button>
          </div>
       </div>
       </div>
  );
